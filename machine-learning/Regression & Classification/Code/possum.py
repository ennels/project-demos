'''AICC 210 - Machine Learning - Triple Threat Lab (possum) - Elijah Walker'''
...
def preview(train):
    ...
    # --- Plotted visual ---
    plt.scatter(train[['footlgth']], train['earconch'])
    plt.xlabel('Foot Length (mm)')
    plt.ylabel('Ear Conch (mm)')
    plt.show()
...
def clean(train):
    ...
    labels = ['Pop']  # Manual. Pop is just a label
...
def split(train):
    ...
    x, y = train[['footlgth']], train['earconch']
...
def results(model, poly, x_train, y_train, x_test, y_test):
    ...
    # --- Plot ---
    x_sorted = x_test.sort_values('footlgth')
...
def main():
    ...
    train = load('possum.csv')
    # preview(train)
    train = clean(train)
    x_train, x_test, y_train, y_test = split(train)
    # pick_degree(x_train, y_train)  # This is what I used to choose the degree
